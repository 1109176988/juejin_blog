...
<template>
    <div class="argicle-lists">
        <div class="arts-item" v-for="item in props.articles" @click="toDetail(item)">
            <div class="art-meta fx">
                {{ item.user.username }} <i /> {{ getTimer(item.created_at) }}<i />{{ store.getCateLabel(item.category) }}
            </div>
            <div class="artctx-wrapper">
                <div class="info-wrap">
                    ...
                    <div class="handle fx">
                        ...
                        <el-dropdown v-if="item.user?._id == ustore.user_info?._id" trigger="hover"
            ...
            <el-empty v-if="props.articles.length == 0" :image-size="100" description="暂无内容"></el-empty>
        </div>
    </div>
</template>
